...
<div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
        {messages.map((msg, index) => (
          <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[75%] px-3 py-2 rounded-xl text-sm whitespace-pre-line ${msg.role === 'user' ? 'bg-purple-600 text-white' : 'bg-purple-50 text-purple-900'}`}>
              {msg.content}
            </div>
          </div>
        ))}
      ...
      {/* [수정된 입력부] */}
      <div className="px-3 py-4 border-t border-purple-50 bg-white">
        <div className="flex items-center gap-2">
          <input 
            ...
            className="flex-1 text-sm border-2 border-purple-200 rounded-xl px-4 py-3 outline-none focus:border-purple-500 transition"
          />
          <button 
            onClick={handleSend} 
            disabled={isLoading}
            className="w-12 h-12 bg-purple-600 text-white rounded-xl flex items-center justify-center hover:bg-purple-700 transition disabled:bg-purple-300"
          >
            <Send size={20} />
          </button>
        </div>
