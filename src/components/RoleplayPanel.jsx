...
// import { getSystemPrompt } from '../prompts'; // <- 이 파일을 가져와야 합니다!
...
function RoleplayPanel({ episodes, competencies }) {
  ...
  // 프롬프트 설정 (상황을 반영한 동적 프롬프트)
  const systemPrompt = `고객 유형: ${customerType}, 상황: ${situation}에 맞게 영업 롤플레잉을 진행해줘.`;

  const handleStart = async () => {
    if (!customerType.trim() || !situation.trim()) return alert('모두 선택해주세요!');
    setStep('chat');
    setIsLoading(true);
  ...
  if (step === 'setup') {
    return (
      <div className="p-6 bg-white rounded-lg shadow-md border-2 border-purple-200 max-w-xl mx-auto">
        ...
        <select className="w-full mb-3 p-3 border-2 border-purple-200 rounded-lg" onChange={(e) => setCustomerType(e.target.value)}>
          ...
          {[...new Set(episodes.map(e => e.customerType1))].map(type => <option key={type} value={type}>{type}</option>)}
        </select>
        <select className="w-full mb-6 p-3 border-2 border-purple-200 rounded-lg" onChange={(e) => setSituation(e.target.value)}>
          ...
          {episodes.filter(e => e.customerType1 === customerType).map(e => <option key={e.id} value={e.situation1}>{e.situation1}</option>)}
        </select>
        <button onClick={handleStart} className="w-full bg-purple-600 text-white font-bold py-3 rounded-lg">시작하기</button>
      </div>
    );
  }
...
<div className="bg-white rounded-2xl shadow-sm border border-purple-100 max-w-xl mx-auto flex flex-col h-[600px]">
      <div className="px-4 py-3 border-b border-purple-50 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
            <span className="text-xs text-purple-400">{userTurnCount}/{MAX_TURNS}</span>
        ...
        {messages.map((msg, index) => (
          <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[75%] px-3 py-2 rounded-xl text-sm ${
              msg.role === 'user' ? 'bg-purple-600 text-white' : 'bg-purple-50 text-purple-900'
            }`}>
              {msg.content}
            </div>
          </div>
      ...
      <div className="flex items-center gap-2 px-3 py-3 border-t border-purple-50">
        ...
        placeholder="메시지 입력..." className="flex-1 text-sm border border-purple-100 rounded-xl px-3 py-2 outline-none focus:border-purple-400" />
